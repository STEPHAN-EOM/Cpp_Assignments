...
void ConvexHull::merge_to_right(const ConvexHull &right){
    ...
    // Split hulls into upper and lower hulls
    ...
    // Add extreme points to both upper and lower hulls
    upper_left_hull.push_back(hull[0]);
    upper_right_hull.push_back(hull[0]);
    lower_left_hull.push_back(hull[0]);
    lower_right_hull.push_back(hull[0]);

    // Find CG as the maximum
    Point max_point = hull[0];
    for (const auto& point : right.hull) {
        if (point.x > max_point.x || (point.x == max_point.x && point.y < max_point.y)) {
            max_point = point;
        }
    }

    // Add the joining segments to the overall outer hull
    hull.clear();
    hull.insert(hull.end(), upper_left_hull.rbegin(), upper_left_hull.rend());
    hull.push_back(max_point);
    hull.insert(hull.end(), upper_right_hull.begin(), upper_right_hull.end());

    // Clear right hull
    ConvexHull& mutableRight = const_cast<ConvexHull&>(right);
    mutableRight.hull.clear();
}
*/
